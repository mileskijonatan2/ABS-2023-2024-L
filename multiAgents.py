...
class MultiAgentSearchAgent(Agent):
    ...
    def __init__(self, evalFn = 'scoreEvaluationFunction', depth = '2', alpha = '0', beta = '0'):
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        # bestScore = max(scores)
        # bestIndices = [index for index in range(len(scores)) if scores[index] == bestScore]
        # bestIndex = random.choice(bestIndices)
        #
        # return legal_actions[bestIndex]
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        return self.max_value(gameState, 0, 0, 0, 0)[1]
    ...
    def max_value(self, gameState, agentIndex, depth, alpha, beta):
        ...
        for action in gameState.getLegalActions(agentIndex):
            successor_state = gameState.generateSuccessor(agentIndex, action)
            v1 = self.value(successor_state, next_agent_index, depth+1, self.alpha, self.beta)
            if v1 > val:
                val = v1
                best_action = action
            if val >= beta:
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def exp_value(self, gameState, agentIndex, depth):
        ...
        for action in gameState.getLegalActions(agentIndex):
            successor_state = gameState.generateSuccessor(agentIndex, action)
            v1 = self.value(successor_state, next_agent_index, depth+1)
            val += v1  # / len(gameState.getLegalActions(agentIndex))
        return val / len(gameState.getLegalActions(agentIndex)), None
